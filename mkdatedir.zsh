...
show_help() {
    ...
    print -P "" >&2
    print -P "Examples:" >&2
    print -P "  script.sh -d" >&2
    print -P "  script.sh -e 2025-03-02" >&2
    print -P "  script.sh -s 2025-03-06" >&2
    print -P "  script.sh --help" >&2
}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -e|--end-date) [ -n "$2" ] && end_date=$(date -I -d "$2") || {print -P "%F{009} [Error] Invalid args %f" >&2; show_help; exit 1}; shift 2;;
        -e=*|--end-date=*) ARG="$1"; end_date=$(date -I -d "${ARG#*=}"); unset ARG; shift;;
        -h|--help) show_help; exit 0;;
        -s|--start-date) [ -n "$2" ] && start_date=$(date -I -d "$2") || {print -P "%F{009} [Error] Invalid args %f" >&2; show_help; exit 1}; shift 2;;
        -s=*|--start-date=*) ARG="$1"; start_date=$(date -I -d "${ARG#*=}"); unset ARG; shift;;
        --) shift;  POSITIONAL_ARGS+=("$@"); set --;;
        -*) print -P "%F{009}[ERROR] Unknown option $1%f" >&2; show_help; exit 1;;
        --*) print -P "%F{009}[ERROR] Unknown option $1%f" >&2; show_help; exit 1;;
